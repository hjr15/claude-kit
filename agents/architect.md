...
5. Hunt for over-engineering and cut it (YAGNI) — bias to the one-file fix over the multi-layer design unless the complexity is earned.
6. Flag ADR-worthy decisions only when they pass all three tests: hard to reverse, surprising without context, and the result of a real trade-off. Instance config / specific values are NOT ADRs; durable decisions belong in long-lived docs.
