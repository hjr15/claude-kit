...
3. Inspect output, not exit codes: a passing exit, `--passWithNoTests`, a silent skip, or an unmatched selector can mask zero work.
4. Apply the rollback test where a fix is claimed: would the symptom return if the change were reverted? If you can't tell, it isn't proven.
